...
public class AccountPrincipal implements OAuth2User, UserDetails {
  ...
  public AccountPrincipal(Long id, String email, Collection<? extends GrantedAuthority> authorities) {
    this.id = id;
    this.email = email;
  ...
  public static AccountPrincipal create(Account account, Map<String, Object> attributes) {
    List<GrantedAuthority> authorities = Collections.singletonList(new SimpleGrantedAuthority(Role.GUEST.getKey()));
    AccountPrincipal accountPrincipal = new AccountPrincipal(account.getId(), account.getEmail(), authorities);
